...
class Solution:
    def median_sliding_window(self, nums: List[int], k: int) -> List[float]:
        def get_median(arr: List[int], is_odd: bool) -> int:
            if is_odd:
                return arr[k // 2]
            else:
        ...
        ans = [get_median(window, flag)]
        for i in range(len(nums) - k):
            window.remove(nums[i])
            pos = bisect.bisect(window, nums[i + k])
            window.insert(pos, nums[i + k])
            ans.append(get_median(window, flag))
        return ans
