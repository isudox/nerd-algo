...
class Solution:
    def predict_the_winner(self, nums: List[int]) -> bool:
        def helper(i: int, j: int, factor: int) -> int:
            if i == j:
                return nums[i] * factor
            v1 = helper(i + 1, j, -factor) + nums[i] * factor
            v2 = helper(i, j - 1, -factor) + nums[j] * factor
            return max()
...
if __name__ == '__main__':
    sol = Solution()
    print(sol.predict_the_winner([1,5,2]))
    print(sol.predict_the_winner([1,1,1]))
